...
class ChatUIManager {
    ...
    async handleNewMessage(message, userRole) {
        ...
        if (userRole === 'assistant') {
            await this._handleNewAssistantMessage(message);
            return;
        }
    ...
    async _handleNewAssistantMessage(message) {
        let modifiedContent = message;
        for (let fn of hooks.beforeUserMessageAdd) {
            const result = fn(modifiedContent, 'assistant');
            if (result === false) return;
            if (typeof result === 'string') modifiedContent = result;
        }
        const newMessage = this.addMessage({ role: 'assistant', content: modifiedContent });
        hooks.afterMessageAdd.forEach(fn => fn(newMessage));
    }

    async generateAIResponse(options = {}, targetChatlog = this.chatBox.chatlog) {
        log(3, 'ChatUIManager: generateAIResponse called');
        ...
        try {
            const payload = this._prepareApiPayload(targetChatlog, mergedSettings);
            if (!payload) return; // e.g., no messages to send

            await this._streamAndProcessResponse(payload, targetMessage);
        } catch (error) {
            this._handleApiResponseError(error, targetChatlog);
        } finally {
            this._finalizeResponse(targetChatlog, mergedSettings);
        }
    }
    ...
    _prepareApiPayload(targetChatlog, mergedSettings) {
        let payload = {
        ...
        return hooks.beforeApiCall.reduce((p, fn) => fn(p, this.chatBox) || p, payload);
    }

    async _streamAndProcessResponse(payload, targetMessage) {
    ...
    _handleApiResponseError(error, targetChatlog) {
        if (error.name === 'AbortError') {
            log(3, 'ChatUIManager: Response aborted by user.');
        ...
        log(1, 'ChatUIManager: generateAIResponse error', error);
    ...
    _finalizeResponse(targetChatlog, mergedSettings) {
