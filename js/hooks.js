...
export const hooks = {
    ...
    beforeApiCall: [], // Called before making the API call, allows modifying the payload. Args: (payload, uiManager, chatLogManager) => modifiedPayload.
    onChunkReceived: [], // Called when a streaming chunk is received from the API. Args: (deltaContent).
    onMessageComplete: [], // Called when an assistant message is fully streamed/completed. Args: (messageObj, uiManager, chatLogManager).
    ...
    onRenderMessageControls: [], // Called to add controls (buttons) to a message element. Args: (containerEl, message, uiManager, chatLogManager).
